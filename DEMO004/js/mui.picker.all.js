 ...
 * title:标题
 * dtpicker 添加新参数
 * showWeek：在显示day的时候，是否需要同时显示星期，默认不显示
 */
	...
	var PopPicker = $.PopPicker = $.Class.extend({
		...
		init: function(options) {
			...
			if(self.options.title){
				self.panel.querySelector('.title').innerHTML = self.options.title;
			}
		},
...
(function($, document) {
	...
	var DtPicker = $.DtPicker = $.Class.extend({
		init: function(options) {
			...
			//添加标题
			if(options.title){
				_picker.querySelector('.title').innerHTML = options.title;
			}
		...
		_createDay: function(current) {
			...
			if (options.customData.d) {
				...
				var y = parseInt(this.ui.y.picker.getSelectedValue());
				var m = parseInt(this.ui.m.picker.getSelectedValue());
				for (; d <= maxDay; d++) {
					var val = self._fill(d);
					if(self.options.showWeek){
						dArray.push({
							text: val +' '+ this._getWeek((new Date(y+'-'+m+'-'+d)).getDay()),
							value: val
						});
					}
					else{
...
//			if (beginDate instanceof Date && !isNaN(beginDate.valueOf())) { //设定了开始日期
			if (beginDate && !isNaN(beginDate.valueOf())){
